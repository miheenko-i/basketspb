...
export function BookingForm({ venues, venueId, program, audience, onVenueChange, onProgramChange, onAudienceChange, onClose }: Props) {
  ...
  async function handleSubmit(event: SubmitEvent<HTMLFormElement>) {
    ...
    if (String(data.get('Страница или контакт') ?? '').trim()) data.set('Связаться через', contactMethod);
  ...
  return <>
    ...
    <form onSubmit={handleSubmit} className="booking-form" aria-busy={status === 'sending'}>
      ...
      <fieldset disabled={status === 'sending'} className="booking-fields">
        ...
        <label htmlFor="booking-age">Возраст ученика *<Input id="booking-age" name="Возраст ученика" type="number" inputMode="numeric" placeholder="Полных лет" required min={1} max={99} step={1} className="booking-input" onChange={event => { setStudentAge(event.target.value); const age = Number(event.target.value); if (age > 0) onAudienceChange(age < 14 ? 'child' : 'adult'); }}/></label>
        {isMinor && <label htmlFor="booking-representative" className="booking-full">Имя и фамилия родителя или законного представителя *<Input id="booking-representative" name="ФИО законного представителя" autoComplete="name" required maxLength={120} className="booking-input"/><span className="booking-representative-note">Заявку на ребёнка заполняет представитель. Ниже укажите свои контакты.</span></label>}
        ...
        <label htmlFor="booking-email">Ваша почта <span className="optional-label">Необязательно</span><Input id="booking-email" name="email" type="email" autoComplete="email" placeholder="mail@example.ru" maxLength={160} className="booking-input"/></label>
        <fieldset className="booking-contact booking-full">
          <legend>Страница ВК или Telegram <span className="optional-label">Необязательно</span></legend>
          ...
          <Input id="booking-contact" name="Страница или контакт" placeholder={contactMethod === 'Telegram' ? '@username или номер телефона' : 'Ссылка на страницу ВК'} maxLength={200} className="booking-input"/>
        </fieldset>
        ...
        <label htmlFor="booking-source" className="booking-full">Откуда о нас узнали <span className="optional-label">Необязательно</span><Select value={source} onValueChange={setSource}><SelectTrigger id="booking-source" className="booking-select"><SelectValue placeholder="Выберите вариант"/></SelectTrigger><SelectContent>{referralSources.map(value => <SelectItem value={value} key={value}>{value}</SelectItem>)}</SelectContent></Select></label>
        <label htmlFor="booking-message" className="booking-full">Сообщение <span className="optional-label">Необязательно</span><Textarea id="booking-message" name="Сообщение" placeholder="Уточните удобное время или задайте вопрос о занятиях" maxLength={3000} rows={3} className="booking-input booking-textarea"/></label>
